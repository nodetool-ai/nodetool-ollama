...
class AggregateEmbeddings(GraphNode):
    ...
    model: types.LlamaModel | GraphNode | tuple[GraphNode, str] = Field(default=types.LlamaModel(type='llama_model', name='', repo_id='', modified_at='', size=0, digest='', details={}), description=None)
...
class Ollama(GraphNode):
    ...
    messages: list[types.Message] | GraphNode | tuple[GraphNode, str] = Field(default=[], description='History of messages to send to the model.')
    context_window: int | GraphNode | tuple[GraphNode, str] = Field(default=4096, description='The context window size to use for the model.')
