...
class AggregateEmbeddings(BaseNode):
    ...
    async def process(self, context: ProcessingContext) -> NPArray:
        ...
        if self.aggregation == self.EmbeddingAggregation.MEAN:
            aggregation = np.mean(embeddings, axis=0)
        elif self.aggregation == self.EmbeddingAggregation.MAX:
            aggregation = np.max(embeddings, axis=0)
        elif self.aggregation == self.EmbeddingAggregation.MIN:
            aggregation = np.min(embeddings, axis=0)
        elif self.aggregation == self.EmbeddingAggregation.SUM:
